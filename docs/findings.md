...
| `noise` — это `Simplex.raw2d(0, x, y)`. Пока не перенесён: выдаёт диагностику вместо правдоподобного вранья | `logic/LogicOp.java` | ✓ `ops.test.js` |
...
Требуют проверки по исходникам библиотеки **arc**, которой нет в текущем чекауте Mindustry.

| Вопрос | Почему важно |
| --- | --- |
| Порядок аргументов `Angles.angle(x, y)` и точная нормализация | От него зависит, что вернёт `op angle` для отрицательных координат. Сейчас реализовано как `atan2(y, x)`, приведённое к `[0, 360)` |
| Что именно принимает `Strings.parseDouble` | От этого зависит, будет ли `1e5` или `5f` числом или именем переменной. Сейчас взята консервативная регулярка |
| Точная реализация `Simplex.raw2d` | Нужна для `op noise`, пока не перенесён |
| Начальное состояние `Rand` в arc | Игра засеивает случайно при старте, совпасть значение в значение невозможно. У нас фиксированный seed ради воспроизводимости |
