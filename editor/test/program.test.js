...
test('строковый литерал переживает чистку, а кавычки внутри экранируются', () => {
    /*
     * До v160 кавычка внутри строки менялась на апостроф; с v160 `LStatement.sanitize`
     * её экранирует, а уже написанное экранирование не трогает.
     */
    assert.equal(sanitize('"текст с пробелом"'), '"текст с пробелом"')
    assert.equal(sanitize('"а "вот" так"'), '"а \\"вот\\" так"')

    // Уже написанные последовательности сохраняются как есть
    assert.equal(sanitize('"строка\\nвторая"'), '"строка\\nвторая"')
    assert.equal(sanitize('"знак \\u0041"'), '"знак \\u0041"')
})

test('в обычном значении подменяются все разделители, а не только пробел', () => {
    // LStatement.sanitize: ';' в 's', кавычка в апостроф, пробел, табуляция и '#' в '_'
    assert.equal(sanitize('мой запас'), 'мой_запас')
    assert.equal(sanitize('мой\tзапас'), 'мой_запас')
    assert.equal(sanitize('мой#запас'), 'мой_запас')
    assert.equal(sanitize('a;b'), 'asb')

    // Одиночный разделитель вписать нельзя вовсе
    for (const char of ['"', ';', ' ', '\t', '#']) assert.equal(sanitize(char), 'invalid', char)
})
